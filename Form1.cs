...
namespace asgn5v1
{
	...
	public class Transformer : System.Windows.Forms.Form
	{
		...
		public Transformer()
		{
			...
			Text = "COMP 4560:  Assignment 5 (200830) (Nam Dang)";
			ResizeRedraw = true;
		...
		private void InitializeComponent()
		{
            ...
            this.toolBar1.Location = new System.Drawing.Point(683, 0);
            this.toolBar1.Name = "toolBar1";
            this.toolBar1.ShowToolTips = true;
            this.toolBar1.Size = new System.Drawing.Size(24, 357);
		...
		private void toolBar1_ButtonClick(object sender, System.Windows.Forms.ToolBarButtonClickEventArgs e)
		{
            ...
            {
                timer.Stop();
                if (rotation != "x" || timer.Enabled == false)
                {
                    rotation = "x";
                    timer.Start();
                }
            ...
            {
                timer.Stop();
                if (rotation != "y" || timer.Enabled == false)
                {
                    rotation = "y";
                    timer.Start();
                }
            ...
            {
                timer.Stop();
                if (rotation != "z" || timer.Enabled == false)
                {
                    rotation = "z";
                    timer.Start();
                }
        ...
        private void InitializeImage()
        {
            ...
            resetScaling(scalingRate, scalingRate, scalingRate);
        ...
        private void Translation(double x, double y, double z)
        {
            ...
            multiply_Matrix(ctrans, ctrans, translationMatrix, 4);
        }
        private void Reflection(char c)
        {
            ...
            multiply_Matrix(ctrans, ctrans, reflectingMatrix, 4);
        }
        private void Scaling(double x, double y, double z)
        {
            double[,] scaled = new double[4, 4];

            setIdentity(scaled, 4, 4);

            scaled[0, 0] = x;
            scaled[1, 1] = y;
            scaled[2, 2] = z;

            double[,] tNet = calculateTnet(scaled);

            multiply_Matrix(ctrans, ctrans, tNet, 4);
        }
        private void resetScaling(double x, double y, double z)
        {
            double[,] scaled = new double[4, 4];
            ...
            multiply_Matrix(ctrans, ctrans, scaled, 4);
        }
        private void RotationX(double radians)
        {
            ...
            double[,] tNet = calculateTnet(rotMat);
        ...
        double [,] calculateTnet(double [,] mat)
        {
            double[,] translationMatrix = new double[4, 4];
            ...
            multiply_Matrix(tNet, translationMatrix, mat, 4);

            translationMatrix[3, 0] = x;
            translationMatrix[3, 1] = y;
            translationMatrix[3, 2] = z;

            multiply_Matrix(tNet, tNet, translationMatrix, 4);
            return tNet;
        }
        private void Sheer(double ratio)
        {
            ...
            Translation(-x, -y, -z);
            multiply_Matrix(ctrans, ctrans, shearMat, 4);
            Translation(x, y, z);
        }
        private void multiply_Matrix(double[,] output, double[,] inputOne, double[,] inputTwo, int maxSize)
        {
